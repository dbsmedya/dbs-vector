...
def test_list_engines_returns_metadata_per_engine():
    out_str = discovery_mod._build_engine_listing(None)
    out = json.loads(out_str)
    names = {e["name"] for e in out}
    assert names == {"md", "sql"}


def test_list_engines_includes_profile_knobs():
    out = json.loads(discovery_mod._build_engine_listing(None))
...
def test_list_engines_marks_unloaded_engines(_clean_state):
    out = json.loads(discovery_mod._build_engine_listing(None))
    for entry in out:
        assert entry["loaded"] is False


def test_list_engines_marks_loaded_engines(_clean_state, monkeypatch):
...
def test_list_engines_works_with_partial_services_map(_clean_state, monkeypatch):
...
def test_list_engines_includes_mcp_tool_name():
    out = json.loads(discovery_mod._build_engine_listing(None))
    md = next(e for e in out if e["name"] == "md")
    assert md["mcp_tool"] == "search_md"


def test_list_engines_advertises_read_tool_only_for_documents():
    out = json.loads(discovery_mod._build_engine_listing(None))
    md = next(e for e in out if e["name"] == "md")
    sql = next(e for e in out if e["name"] == "sql")
...
def test_list_engines_scoped_to_subset():
    out = json.loads(discovery_mod._build_engine_listing({"md"}))
    names = {e["name"] for e in out}
    assert names == {"md"}
...
def test_register_discovery_tool_raises_on_name_clash(fresh_mcp):
    ...
    with pytest.raises(RuntimeError, match="already registered with sentinel"):
        discovery_mod.register_discovery_tool(fresh_mcp)


def test_register_discovery_tool_different_scope_raises(fresh_mcp):
    """Re-registering list_engines with a DIFFERENT engine_names scope on the
    same FastMCP instance must raise, not silently keep the stale scope."""
    discovery_mod.register_discovery_tool(fresh_mcp, engine_names={"md"})
    with pytest.raises(RuntimeError, match="already registered with sentinel"):
        discovery_mod.register_discovery_tool(fresh_mcp, engine_names={"sql"})


def test_register_discovery_tool_same_scope_idempotent(fresh_mcp):
    discovery_mod.register_discovery_tool(fresh_mcp, engine_names={"md"})
    count_first = len(fresh_mcp._tool_manager.list_tools())
    discovery_mod.register_discovery_tool(fresh_mcp, engine_names={"md"})
    count_second = len(fresh_mcp._tool_manager.list_tools())
