...
def _patch_search_path(engines: dict):
    """Patch settings + dependencies so the search command runs without I/O."""
    deps = MagicMock()
    deps.embedder = MagicMock()
    deps.store = MagicMock()
    return [
        patch("dbs_vector.cli.settings", _settings_with(engines)),
...
def test_min_time_forwarded_for_sql_family_engines(runner, engine_name, mapper_type):
    ...
    patches, MockService = _patch_search_path({engine_name: mapper_type})
...
def test_min_time_ignored_for_document_engines(runner):
    ...
    patches, MockService = _patch_search_path({"md": "document"})
    with patches[0], patches[1], patches[2]:
        MockService.return_value.execute_query.return_value = []
...
Expected: the four parametrized cases FAIL for `sql-api`, `sql-granite`, `sql-api-granite` (current code uses `engine_name == "sql"` literal); the `md` case passes.
