...
- Add a top-level config option `system.preload_engines: list[str]` to filter which engines `initialize_services()` actually loads. **Default: `["md", "sql"]`** — matches the engines the existing hardcoded routes already exposed pre-Phase-2, so out-of-the-box `serve`/`mcp` returns to the original two-model startup cost. Operators who want all engines warm at startup set `preload_engines` explicitly to the full list (e.g. `["md", "sql", "sql-api", "md-granite", "sql-granite", "sql-api-granite"]`).
